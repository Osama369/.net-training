...
namespace aiPriceGuard.Api.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController(UserManager<IdentityUser> userManager, SignInManager<IdentityUser> signInManager, AMDbContext aMDbContext, Context context, IConfiguration configuration, IHttpContextAccessor httpContextAccessor, IEmailService emailService, IWebHostEnvironment _environment )
        {
        ...
        public async Task<IActionResult> loginUser(Tenants tenant)
        {
            ...
            if (user != null)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    {
                        _AMDbContext.Tenants.FirstOrDefault(x => x.tenantID == item.tenantID).tenantName,
                        item.tenantID,
                        item.isPrimary
                    }).ToList();
                    ...
                    return Ok(new { tenantNames, idToken = token });
       ...
       // [HttpPost]
        ...
        public async Task<IActionResult> confirmTenantVerification([FromBody] Tenants tenant)
        {
            ...
            else
            {
                ...
                restoreDatabase("OCR_" + lst[0].tenantID, lst[0].tenantID);
        ...
        public string createDataBase(Tenants tenant)
        {
            string connString = _configuration.GetConnectionString("MyConnection");
            SqlConnection sqlconn = new SqlConnection(connString);
            string queryy = "Create database OCR_" + tenant.tenantID;
            SqlCommand cmd = new SqlCommand(queryy, sqlconn);
            sqlconn.Open();
            cmd.ExecuteNonQuery();
            sqlconn.Close();
            ...
            { InitialCatalog = "OCR_" + tenant.tenantID };
