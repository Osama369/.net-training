...
@Component({
  selector: 'app-add-new-scheme-d',
  templateUrl: './add-new-scheme-d.component.html',
  styleUrls: ['./add-new-scheme-d.component.scss']
})
export class AddNewSchemeDComponent {
  ...
  async ngOnInit(): Promise<void> {
   ...
   // var data = await lastValueFrom(this.sharedServices.getProducts$());
    this.sharedServices.getProducts$().subscribe(list=>{
      this.productlist=(list as{[key:string] :any} )["enttityDataSource"];
  ...
  onDepartmentChange() {
    var depID= this.SelectedDepartment.depID;
    var categoryID= this.SelectedBrand.categoryID;
    ...
    // if(depID!==0 && categoryID===0)
    // {
        this.filterProductlist= this.productlist.filter(x=>x.depID===depID && x.prodGrpID===categoryID)
    //}
    // if(depID===0 && categoryID!==0)
    // {
    //     this.filterProductlist= this.productlist.filter(x=>x.prodGrpID===categoryID)
    // }
  ...
  saveSchemes()
  {
    ...
    this.schemesList= this.schemesList.filter(x=>
      !(x.startQty===0 && x.maxQty===0 && x.maxAmount===0 && x.discount===0)
    )
