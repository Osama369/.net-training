...
@Component({
  selector: 'app-stock-shortage',
  templateUrl: './stock-shortage.component.html',
  styleUrls: ['./stock-shortage.component.scss']
})
export class StockShortageComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.invoiceService.getInvoicesList(38).subscribe(invoices => {
    ...
    {
      this.authService.checkPermission('StockShortageDelete').subscribe((x:any)=>{
        if(x)
        {
  ...
  exportExcel() {
    ...
    import('xlsx').then((xlsx) => {
      ...
      this.saveAsExcelFile(excelBuffer, "Stock Shortage");
    });
  }
