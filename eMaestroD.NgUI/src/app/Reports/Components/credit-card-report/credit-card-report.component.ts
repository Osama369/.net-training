...
export class CreditCardReportComponent {
  ...
  ngOnInit(): void {
    ...
    this.genericService.GetAllCreditCards().subscribe({
      next: (banks) => {
        this.creditCards = (banks as { [key: string]: any })["enttityDataSource"];
        this.creditCards.unshift({
          COAID : 0,
          parentCOAID : 0,
          acctName : "---ALL---",
    ...
    this.SelectedcreditCard = {COAID:  0, acctName: "---ALL---"};
      },
      error: (response) => {
  ...
  submit()
  {
    ...
    let d2 =  this.datePipe.transform(this.DateTo, "yyyy-MM-dd");
      ...
      this.data = [];
      let locID = 0;
      this.reportService.runReportWith3Para("CreditCard",d1,d2,this.SelectedcreditCard.COAID,locID).subscribe(data => {
