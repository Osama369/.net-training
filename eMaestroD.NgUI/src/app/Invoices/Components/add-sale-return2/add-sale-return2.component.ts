...
@Component({
  selector: 'app-add-sale-return2',
  templateUrl: './add-sale-return2.component.html',
  styleUrls: ['./add-sale-return2.component.scss'],
  providers:[ConfirmationService,MessageService]
})
export class AddSaleReturn2Component implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.reportSettingService.GetInvoiceReportSettings().subscribe(rpt=>{
      this.reportSettingItemList = rpt.filter(x=>x.screenName.toLowerCase() == 	"sale return");
    })
    ...
    this.customersService.GetAllCustomerIncludeWalkIn().subscribe({
      next: (customers) => {
            ...
            this.invoicesService.getOneInvoiceDetail(this.EditVoucherNo).subscribe(invoices => {
              ...
              invoices.forEach((elem,index) => {
                ...
                else if(elem.COAID == 137 && elem.txTypeID == 5)
                {
                  ...
                  this.productlist[i].amount  =elem.debitSum - elem.discountSum;
                  this.productlist[i].discount  = elem.checkNo;
                  this.productlist[i].crtDate = elem.crtDate;
                  this.SubTotal += elem.debitSum - elem.discountSum;
                  totalQty += elem.qty;
    ...
    else
    {
      ...
      for (let i = 0; i < this.productlist.length; i++) {
        ...
        else
        {
          ...
          this.gl[j].paidSum = (this.productlist[i].sellRate/100*this.productlist[i].discount)*this.productlist[i].qty;
          ...
          this.gl[j].unitPrice = parseFloat(this.productlist[i].sellRate);
