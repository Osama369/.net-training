...
export class JournalVoucherComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        // })
        this.invoiceService.getInvoicesList(8).subscribe(invoices => {
    ...
    deleteView(invoiceNo:any)
    {
      ...
      if (confirm("Are you sure you want to delete this Journal Voucher?") == true) {
        this.authService.checkPermission('JournalVoucherDelete').subscribe((x:any)=>{
          if(x)
          {
            this.invoiceService.deleteJournalVoucher(invoiceNo).subscribe({
              next: x =>{
                this.toastr.success("Journal voucher has been successfully deleted!");
                this.invoiceService.getInvoicesList(8).subscribe(invoices => {
                  this.invoices = invoices;
                  this.loading = false;
              });
