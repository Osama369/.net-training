...
export class ReceiptVoucherComponent implements OnInit {
  constructor(private invoiceService: InvoicesService,
    ...
    ngOnInit() {
        this.invoiceService.getInvoicesList(6).subscribe(invoices => {
    ...
    deleteView(invoiceNo:any)
    {
      if (confirm("Are you sure you want to delete this Receipt?") == true) {
        this.authService.checkPermission('ReceiptVoucherDelete').subscribe((x:any)=>{
          if(x)
          {
            this.invoiceService.deleteReceiptVoucher(invoiceNo).subscribe({
              next: x =>{
                this.toastr.success("Receipt Voucher has been successfully deleted!");
                this.invoiceService.getInvoicesList(6).subscribe(invoices => {
                  this.invoices = invoices;
                  this.loading = false;
              });
    ...
    OpenLedger() {
      ...
      const url = this.router.serializeUrl(
        this.router.createUrlTree([`./../Reports/PartyLedgerReport`])
      );
