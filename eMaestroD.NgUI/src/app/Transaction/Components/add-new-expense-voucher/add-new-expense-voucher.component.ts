...
@Component({
  selector: 'app-add-new-expense-voucher',
  templateUrl: './add-new-expense-voucher.component.html',
  styleUrls: ['./add-new-expense-voucher.component.scss']
})
export class AddNewExpenseVoucherComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.genericService.GetAllCoaofLevel2().subscribe({
      next: (CoaAccountList) => {
        this.CoaAccountList = CoaAccountList.filter(x=>x.parentAcctType == 'Expense');
    ...
    this.primengConfig.ripple = true;
      if(this.EditVoucherNo != undefined)
      {
        ...
        this.isEdit = true;
          ...
          let i = -1;
          this.voucherList = invoices.filter(x=>x.debit > 0 && x.credit ==0);
          let list =invoices.filter(x=>x.credit > 0);
          this.editVoucherGLID = invoices.filter(x=>x.credit > 0)[0].GLID;
          ...
          this.cdr.detectChanges();
            ...
            this.glComments = invoices[0].masterEntryComment;
  ...
  saveInvoice()
  {
    if(this.TotalBalance == 0) {
      this.toastr.error("Please fill alteast one field!");
      this.onEnterComplex(0);
    }
    ...
    else
    {
      ...
      this.voucherList.forEach(element => {
        ...
        this.voucherList[0].masterEntryComment = this.glComments;
        ...
        {
          this.voucherList[this.voucherList.length-1].COAID = 80;
          this.voucherList[this.voucherList.length-1].parentCOAID = 23;
        }

        this.voucherList[this.voucherList.length-1].parentAccountName = "";
        this.voucherList[this.voucherList.length-1].ChildAccountName = "";
        this.voucherList[this.voucherList.length-1].credit = this.TotalBalance;
        if(this.EditVoucherNo != undefined)
        {
          this.voucherList[this.voucherList.length-1].GLID =  this.editVoucherGLID;
        }

        this.savebtnDisable = true;
        this.invoicesService.SaveExpenseVoucher(this.voucherList).subscribe({
          next: (Gl) => {
            this.toastr.success("Expense voucher has been successfully Added!");
            this.CancelInvoice();
            this.close();
          },
          error: (response) => {
