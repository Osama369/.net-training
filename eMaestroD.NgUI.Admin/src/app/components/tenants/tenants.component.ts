...
export class TenantsComponent implements OnInit {
  constructor(
    ...
    sendEmailToTenant(tenantlist:Tenants)
    {
      let tenants = this.tenantlist.filter(x=>x.email == tenantlist.email);
      this._TenantService.sendEmailToTenant(tenants).subscribe({
          next:(result)=>{
              this._ToastrService.success("Email Has Been Successfully Sended To "+tenantlist.email);
          },
          error:(err)=>{
    ...
    ChangeTenantToSuspended(tenant:Tenants)
    {
      this._TenantService.SuspendTenant(tenant).subscribe({
        next:(result)=>{
            // this._ToastrService.success("Tenant Successfully Suspended");
        },
        error:(err)=>{
            this._ToastrService.error(err.error);
        }
  ...
  showApplication(tenant: any) {
    const baseUrl = 'https://app.emaestroAccounts.com';
    let tenantUrl = baseUrl;

    // Append email as a query parameter
    if (tenant && tenant.email) {
      tenantUrl += `?email=${encodeURIComponent(tenant.email)}`;
    }
