...
export class AddProductComponent {
  ...
  async ngOnInit(): Promise<void> {
    ...
    // console.error('this', this.SelectedSupplier);
    this.disableProdID = false;
    if(this.EditProdID)
    {
      this.disableProdID = true;
  ...
  try {
    this.suppliers.GetAllSuppliers().subscribe(supplier=>{
     
      this.filteredSuppliers = (supplier as { [key: string]: any })["enttityDataSource"];
      // this.cols = (supplier as { [key: string]: any })["entityModel"];
    
    });
    
    this.productlist[0].ProductBarCodes =[];
...
tempSuppliers:Supplier[]=[];
 selectedSupplierList :any[] = [];
  async getEditProductDetail()
  {
    const data = this.productService.GetOnProductDetails(localStorage.getItem('comID'),this.EditProdID).subscribe(x=>{
      this.productlist[0]= x;
      console.log('EditProduct:',this.productlist[0])
      // this.SelectedSupplier = x.vendID;
      
      this.suppliers.GetAllSuppliers().subscribe(y=>
      {
      this.tempSuppliers = (y as { [key: string]: any })["enttityDataSource"];
    
      this.tempSuppliers.forEach(element => {
       
        this.productlist[0].supplierIDList?.forEach(x=>{
            if(x==element.SupplierId){
              this.selectedSupplierList.push(element);
            }
        });
        // if(this.productlist[0].vendID ===element.SupplierId){
        //   this.SelectedSupplier = element;
        // }
      });
        
      })
      console.log('selectSuppList:',this.tempSuppliers);
     
    
      this.SelectedSupplier = this.selectedSupplierList;
     

    });
    //  this.suppliers.GetAllSuppliers().subscribe(supp=>{
    //   let supplierList = (supp as { [key: string]: any })["enttityDataSource"];
    //   this.SelectedSupplier =supplierList.find(x=> x.SupplierId == data.vendID);
    //   console.log('retSupp:',supplierList);
    //   console.log('retSelectedSupp:',this.SelectedSupplier);
...
onRowEditCancel(product:any, editing:any) {
  ...
  saveProduct()
  {
    ...
    {
      this.toastr.error("Please provide Select Supplier.");
    ...
    else
    {
      ...
      let suppIDLIST :number[]=[];
      this.SelectedSupplier.forEach(x=>{
        console.log('x:',x.SupplierId);
        suppIDLIST.push(x.SupplierId);
          // this.productlist[0].supplierIDList.push(x.SupplierId);
      });
      this.productlist[0].supplierIDList = suppIDLIST;
      console.log('selectedSupplier:',this.productlist[0].supplierIDList);
      ...
      // this.productlist[0].IsUpdate =false;
      console.log('product:',this.productlist[0]);
      this.productService.saveProduct(this.productlist[0]).subscribe({
        next: (prd:any) => {
          // this.sharedDataService.updateProducts$(prd);
           

          this.sharedService.UpsertProduct(prd);
